<script lang="ts">
	...
	let resourceTypes: Array<{ id: string; name: string }> = $state([]);
	...
	async function bookSuggested(resourceId: string, suggestedStartAt: string, suggestedEndAt: string) {
		...
		purpose = '';
		await Promise.all([search(), loadBookings()]);
		bookingIdLoading = '';
	}
	...
	async function loadTypes() {
		...
		if (!res.ok) {
			error = data.error ?? 'Failed to load resource types';
			resourceTypes = [];
			loadingTypes = false;
			return;
		}
	...
	async function cancel(id: string) {
		...
		await Promise.all([loadBookings(), search()]);
	}
...
<div class="page">
	<header class="hero">
		...
		<h1>Book what you need without fighting a calendar.</h1>
		<p class="sub">
			Pick a resource type, choose a time, and book from what’s actually available.
		</p>
	</header>
				...
				{#each available as resource (resource.id)}
					<article class="card">
						<div class="card-top">
							<div>
								<h3>{resource.name}</h3>
								{#if resource.location}<p class="muted">{resource.location}</p>{/if}
							</div>
						</div>
	...
	{#if available.length === 0 && suggestions.length > 0}
		<div class="empty">
			Nothing is available for that time. Here are the next best options.
		</div>
			...
			<div class="grid">
				{#each myBookings as booking (booking.id)}
					<article class="card">
						...
						{#if booking.purpose}<p>{booking.purpose}</p>{/if}
